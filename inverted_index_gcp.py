...
class MultiFileReader:
    ...
    def read(self, locs, n_bytes, blob_name, bucket_name='207653312_313689804'):
        b = []
        for f_name, offset in locs:
            if f_name not in self._open_files:
                GCSFS = gcsfs.GCSFileSystem()
                self._open_files[f_name] = GCSFS.open(f'gs://207653312_313689804/{blob_name}/{f_name}', 'rb')
#             if f_name not in self._open_files:
#                 client = storage.Client()
#                 for blob in client.list_blobs(bucket_name):
#                     if blob.name.endswith(f_name):
...
class InvertedIndex:  
    ...
    def read_posting_list(self, w, blob_name, bucket_name='207653312_313689804'):
        locs = self.posting_locs[w]
        df = self.df.get(w,0)
        with closing(MultiFileReader()) as reader:
            b = reader.read(locs, df * TUPLE_SIZE, blob_name, bucket_name)
            posting_list = []
            for i in range(self.df[w]):
                doc_id = int.from_bytes(b[i*TUPLE_SIZE:i*TUPLE_SIZE+4], 'big')
                tf = int.from_bytes(b[i*TUPLE_SIZE+4:(i+1)*TUPLE_SIZE], 'big')
